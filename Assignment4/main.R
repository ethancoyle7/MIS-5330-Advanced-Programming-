...
#    any of these user-defined functions!vector.maker.num() should have no formal  #
#    arguments. Each time it is called, and it should return a vector of exactly   #
#    5 different random numbers from within the range of 1 to 100. Then test out   #
#    Several times                                                                 #
####################################################################################
...
#    of characters. vector.maker.alph() should have no formal arguments. When it   #
#    is called, it should return a vector of exactly 5 different random letters    #
#    from the 26 lower case letters of the alphabet. Here is a helpful tip: the    #
#    native R object 'letters' returns the 26 lower-case letters of the alphabet   #
#    as character elements. Then try it out several times                          #
#                                                                                  #
####################################################################################
...
vector.maker.alph <- function() {
  ...
  alphabet <- letters # take the values of all the lower case letters and assign to 
                      # alphabet
  # the sample is generated from the alphabet extracting 5 random numbers which is
  # assigned to the unique letters 
  unique_letters <- sample(alphabet, 5) 
  return(unique_letters) # return back to the calling function passing back the 5 
                         # letters
} # end of the function method
#  As specified, try this out multiple times(sticking with 5) this calls the function
# a total of 5 times
...
# Once this is done, to see the values of these display on the screen consecutively
# display 1st test
randomletters1
# display 2nd test
randomletters2
...
# Exercise 3                                                                       #
#                                                                                  #
#    Create a user-defined function "vector.maker.bool" that outputs a vector of   #
#    logical values (TRUE's and FALSE's).vector.maker.bool() should have no formal #
#    arguments. When it is called, it should return a vector of exactly 5 different#
#    random values (mixed) containing both TRUE's AND FALSE's.Test mult. times     #
#                                                                                  #
####################################################################################

# crete function method that generates 5 rnadom values of either true or false and
# returns the 5 generated values back to the calling functon in the testing
vector.maker.bool <- function() {
  unique_bool <- sample(c(TRUE, FALSE), 5) # we generate the values
